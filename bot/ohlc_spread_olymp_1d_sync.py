"""Sync do /ohlc-spread-olymp-1d: OTC Olymp D1 + EURUSD D1 (Dukascopy agg)."""
...
def _otc_asset() -> str:
    return normalize_asset(
        os.environ.get("OHLC_SPREAD_OLYMP_1D_OTC_ASSET", "").strip()
...
def rebuild_olymp_1d_from_hourly(
    *,
    otc_asset: str | None = None,
    days: int | None = None,
    include_today: bool = True,
) -> dict[str, Any]:
    """Agrega ohlc_candles_olymp (1h) → ohlc_candles_olymp_1d."""
    otc_a = normalize_asset(otc_asset or _otc_asset())
    lookback = max(
        1, min(int(days or _env_int("OHLC_SPREAD_OLYMP_1D_SYNC_DAYS", 120)), 800)
    )
    end = datetime.now(timezone.utc)
    start = end - timedelta(days=lookback)

    hourly = fetch_candles_range(
        otc_a, timeframe="1h", table=TABLE_OLYMP, after=start
    )
    ...
    upserted = upsert_candles(daily, table=TABLE_OLYMP_1D) if daily else 0
    return {
        "ok": True,
        "asset": otc_a,
...
def reconcile_eurusd_to_olymp_1d(
    *,
    otc_asset: str | None = None,
    eurusd_asset: str | None = None,
    days: int | None = None,
) -> dict[str, Any]:
    """Pente fino: dias Olymp D1 de sessao sem EURUSD D1 → re-agrega 1h."""
    otc_a = normalize_asset(otc_asset or _otc_asset())
    ...
    otc_win = fetch_candles_range(
        otc_a, timeframe=TIMEFRAME, table=TABLE_OLYMP_1D, after=start
    )
...
def sync_spread_olymp_1d_sources(
    *,
    days: int | None = None,
    pull_otc: bool = True,
    pull_dukascopy: bool = True,
) -> dict[str, Any]:
    """Pente fino D1 Olymp: tip 1h Olymp → agg D1 + EURUSD D1 Dukascopy."""
    otc_a = _otc_asset()
    eu_a = _eurusd_asset()
    lookback = max(
        1, min(int(days or _env_int("OHLC_SPREAD_OLYMP_1D_SYNC_DAYS", 120)), 800)
    ...
    result: dict[str, Any] = {
        ...
        "days": lookback,
        "mode": "fine_comb_olymp_1d",
        "timeframe": TIMEFRAME,
        "table_otc": TABLE_OLYMP_1D,
    ...
    if pull_otc:
        try:
            ...
            pull = collector_olymp.pull_now(otc_a, hours=hours)
            tip_n = int((pull.get("pull") or {}).get("upserted") or 0)
            rebuilt_otc = rebuild_olymp_1d_from_hourly(
    ...
    if pull_dukascopy:
        ...
        try:
            recon = reconcile_eurusd_to_olymp_1d(
    ...
    result["otc_summary"] = stored_summary(otc_a, TIMEFRAME, table=TABLE_OLYMP_1D)
...
def backfill_olymp_otc_1d(
    *,
    hours: int | None = None,
    otc_asset: str | None = None,
) -> dict[str, Any]:
    """Puxa historico Olymp 1h e agrega D1."""
    otc_a = normalize_asset(otc_asset or _otc_asset())
    ...
    try:
        if collector_olymp.status().get("asset") != otc_a:
            try:
                ...
                pass
        pull = collector_olymp.pull_now(otc_a, hours=hrs)
        hourly_info = pull.get("pull") if isinstance(pull, dict) else None
    except Exception as exc:  # noqa: BLE001
        tip_error = str(exc)[:300]

    rebuilt = rebuild_olymp_1d_from_hourly(
    ...
    return {
        ...
        "error": tip_error,
        "note": (
            "OTC Olymp D1 e agregado do 1h em ohlc_candles_olymp. "
            "Se fetched=0, puxe historico na ferramenta Olymp 1h antes."
        ),
    }


def backfill_dukascopy_olymp_1d(
    *,
    days: int | None = None,
    match_otc: bool = True,
    otc_asset: str | None = None,
) -> dict[str, Any]:
    """EURUSD D1 casado ao Olymp D1."""
    otc_a = normalize_asset(otc_asset or _otc_asset())
    lookback = max(1, min(int(days or 120), 800))

    if match_otc:
        try:
            oldest = oldest_opened_at(otc_a, TIMEFRAME, table=TABLE_OLYMP_1D)
    ...
    result["otc_table"] = TABLE_OLYMP_1D
    result["ok"] = bool(
