...
class OhlcCollectorOlymp:
    ...
    def _refresh_flags(self) -> None:
        ...
        self._snap["olymp_ok"] = o_ok
        self._snap["olymp_msg"] = o_msg
    ...
    def set_asset(self, asset: str) -> dict[str, Any]:
        ...
        with self._lock:
            if self.is_running():
                raise RuntimeError("Pare o coletor Olymp antes de trocar o ativo.")
            self._asset = a
            self._snap["asset"] = a
    ...
    def set_pair(self, pair: str) -> dict[str, Any]:
        ...
        if not p:
            raise ValueError("Par Olymp invalido.")
        with self._lock:
            if self.is_running():
                raise RuntimeError("Pare o coletor Olymp antes de trocar o par.")
            self._pair = p
            self._snap["pair"] = p
            self._snap["message"] = f"Par Olymp: {p}"
        return self.status()
            ...
            self._snap.update(
                {
                    ...
                    "phase": "starting",
                    "error": None,
                    "message": "Iniciando coletor Olymp…",
                    "total_upserted": 0,
                }
            )
            self._thread = threading.Thread(
                target=self._run,
                name="ohlc-collector-olymp",
                daemon=True,
            )
            self._thread.start()
        return self.status()
    ...
    def pull_now(
        ...
        *,
        ...
        lookback_h = max(
            12,
            int(hours or _env_int("OHLC_OLYMP_LIVE_HOURS", 24 * 14)),
        )
        try:
            self._set(
                phase="manual_pull",
                message=f"Puxada Olymp 1h ({target} / {self._pair})…",
                error=None,
            )
            ...
            self._set(
                ...
                message=f"Puxada Olymp ok: {upserted} velas ({target})",
            )
            return {
                "ok": True,
                "pull": {
            ...
            self._set(
                ...
                error=str(exc),
                message=f"Puxada Olymp falhou: {exc}",
            )
            raise
    ...
    def _pull_and_upsert(self, asset: str, *, hours: int) -> int:
        ...
        if not rows:
            self._set(message="Olymp: 0 candles (par/token?)")
            return 0
    ...
    def _run(self) -> None:
        after_s = _env_int("OHLC_OLYMP_AFTER_HOUR_SECONDS", 120)
        live_poll = _env_int("OHLC_OLYMP_LIVE_POLL_SECONDS", 60)
        live_hours = _env_int("OHLC_OLYMP_LIVE_HOURS", 48)
        align = os.environ.get("OHLC_OLYMP_ALIGN_HOUR", "1").strip() not in (
            "0",
            "false",
            "no",
        )
        # Backfill inicial
        try:
            self._set(phase="backfill", message="Backfill Olymp inicial…")
            n = self._pull_and_upsert(
                self._asset,
                hours=_env_int("OHLC_OLYMP_BACKFILL_HOURS", 24 * 30),
            )
            self._set(message=f"Backfill Olymp: {n} velas")
        except Exception as exc:  # noqa: BLE001
            ...
            try:
                if align:
                    ...
                    if self._stop.is_set():
                        break
                    self._set(phase="hourly", message="Fetch horario Olymp…")
                    self._pull_and_upsert(self._asset, hours=live_hours)
                else:
                    poll = max(60, _env_int("OHLC_OLYMP_POLL_SECONDS", 3600))
                    self._set(
                        phase="polling",
...
collector_olymp = OhlcCollectorOlymp()
